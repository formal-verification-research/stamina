...
use crate::{
	bmc::{bmc::bmc, formula::build_z3_encoding},
	logging::messages::*,
...
pub fn get_bounds(model: AbstractVas, bits: u32) {
	...
	let (mut unroller, init_formula, trans_formula, target_formula) =
	...
	for s in vars.iter() {
		...
		loop {
			solver.reset();
			let bound_formula = &unroller
				.at_all_times_or(&state_var.bvuge(&ast::BV::from_i64(&ctx, bound, bits)), k);
	...
	for s in vars.iter() {
		...
		loop {
			solver.reset();
			let bound_formula = &unroller
				.at_all_times_and(&state_var.bvule(&ast::BV::from_i64(&ctx, bound, bits)), k);
			let combined_formula = ast::Bool::and(&ctx, &[bound_formula, &formula]);
			solver.assert(&combined_formula);
			let status = solver.check();
			if status == SatResult::Sat {
	...
	for s in vars.iter() {
		...
		loop {
			...
			solver.reset();
			let bound_formula = &unroller
				.at_all_times_or(&state_var.bvule(&ast::BV::from_i64(&ctx, bound, bits)), k);
			let combined_formula = ast::Bool::and(&ctx, &[bound_formula, &formula]);
			solver.assert(&combined_formula);
			let status = solver.check();
	...
	for s in vars.iter() {
		...
		loop {
			...
			solver.reset();
			let bound_formula = &unroller
				.at_all_times_and(&state_var.bvuge(&ast::BV::from_i64(&ctx, bound, bits)), k);
			let combined_formula = ast::Bool::and(&ctx, &[bound_formula, &formula]);
			solver.assert(&combined_formula);
			let status = solver.check();
			if status == SatResult::Sat {
	...
	for s in vars.iter() {
		message(&format!(
			"{:<20} {:<10} {:<10} {:<10} {:<10}",
			s, lb_loose[s], lb_tight[s], ub_loose[s], ub_tight[s],
		));
	}
	message(&format!("{}", "=".repeat(80)));
}
