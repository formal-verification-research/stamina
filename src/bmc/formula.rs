...
pub fn build_z3_encoding<'ctx>(
	...
	for i in 0..vars.len() {
		...
		init_constraints.push(Ast::_eq(
			&state_var,
			&ast::BV::from_i64(&ctx, init[0].vector[i], bits),
		));
	}
	...
	for transition in &model.transitions {
		...
		for (i, update) in transition.update_vector.iter().enumerate() {
			...
			let update_constraint = if *update > 0 {
				...
				ast::Ast::_eq(
					next_var,
					&state_var.bvadd(&ast::BV::from_i64(
						&ctx,
				...
				ast::Ast::_eq(
					next_var,
					&state_var.bvsub(&ast::BV::from_i64(
						&ctx,
						(-*update).try_into().unwrap(),
						bits,
					)),
				...
				ast::Ast::_eq(next_var, state_var)
			};
			current_transition_constraints.push(update_constraint);
		}
		// Combine all constraints for this transition
		let current_transition_formula = ast::Bool::and(
			&ctx,
			&current_transition_constraints.iter().collect::<Vec<_>>(),
		);
	...
	let transition_formula =
		ast::Bool::or(&ctx, &transition_constraints.iter().collect::<Vec<_>>());
	// Target formula
	...
	let target_formula = ast::Ast::_eq(
		&state_vars[target_variable_name],
		&ast::BV::from_i64(&ctx, target.target_value.try_into().unwrap(), bits),
	);
