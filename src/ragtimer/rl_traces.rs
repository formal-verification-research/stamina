...
use crate::{dependency::graph::DependencyGraph, logging::messages::*, model::vas_model::VasTransition, ragtimer::trace_trie::{self, TraceTrie}, AbstractVas};
...
fn debug_print_rewards(model: &AbstractVas, rewards: &HashMap<usize, f64>) {
    for (transition_id, reward) in rewards {
        debug_message(&format!("{}.{}: {}", model.get_transition_from_id(*transition_id).unwrap().transition_name, transition_id, reward));
    }
}
...
fn initialize_rewards(model: &AbstractVas, dependency_graph: &DependencyGraph) -> HashMap<usize, f64> {
...
/// For now, it initializes all rewards to zero, then adds DEPENDENCY_REWARD to the reward of any transition
/// that appears in the dependency graph.
fn maintain_dg_rewards(model: &AbstractVas, dependency_graph: &DependencyGraph, rewards: &mut HashMap<usize, f64>) {
    let dep_transitions = dependency_graph.get_transitions();
    for transition in dep_transitions {
        if let Some(reward) = rewards.get_mut(&transition.transition_id) {
...
pub fn generate_traces(model: &AbstractVas, dependency_graph: &DependencyGraph, num_traces: usize) -> Vec<Vec<VasTransition>> {
    ...
    for i in 0..num_traces {
        let (trace, reward) = generate_unique_trace(model, &mut trace_trie, &mut rewards, &mut trace_probability_history, 1);
        update_rewards(&trace, &trace_probability_history, &mut rewards);
        maintain_dg_rewards(model, &dependency_graph, &mut rewards);
        traces.push(trace.iter()
    ...
    debug_message(&format!("Total trace probability sum: {:.6e}", total_probability));

    traces
}

fn generate_unique_trace(model: &AbstractVas, trace_trie: &mut TraceTrie, rewards: &HashMap<usize, f64>, trace_probability_history: &mut Vec<f64>, depth: usize) -> (Vec<usize>, f64) {
    ...
    loop {
        ...
        if trace_length > MAX_TRACE_LENGTH {
            warning(&format!("Trace length exceeded maximum of {}. Stopping generation.", MAX_TRACE_LENGTH));
        ...
        let total_reward: f64 = available_transitions.iter().map(
            |t| rewards.get(t).unwrap_or(&0.0)
        ).sum();
        ...
        for (index, &transition) in shuffled_transitions.iter().enumerate() {
            ...
            if rng.random::<f64>() < selection_probability {
                if let Some(vas_transition) = model.get_transition_from_id(transition) {
                    ...
                    trace_probability *= crn_transition_probability(model, vas_transition, &current_state);
                } else {
    ...
    if trace_trie.contains_or_insert(&trace) {
        ...
        return generate_unique_trace(model, trace_trie, rewards, trace_probability_history, depth+1);
    } 
...
fn update_rewards(trace: &[usize], trace_probability_history: &Vec<f64>, rewards: &mut HashMap<usize, f64>) {
    ...
    for &transition_id in trace {
        if let Some(reward) = rewards.get_mut(&transition_id) {
            *reward += trace_reward;
            ...
            error(&format!("Transition ID {} not found in rewards map.", transition_id));
        }
    }
...
fn get_available_transitions(model: &AbstractVas, current_state: &DVector<i64>) -> Vec<usize> {
    ...
    //     ));
    // }
    let x = model.transitions.iter()
        .filter(|t| {
            t.enabled_bounds.iter().zip(current_state.iter()).all(|(bound, &val)| val >= (*bound).try_into().unwrap())
        })
...
pub fn print_traces_to_file(traces: &Vec<Vec<VasTransition>>, file_path: &str) {
    ...
    for trace in traces {
        let trace_str = trace.iter().map(|t| t.transition_name.clone()).collect::<Vec<String>>().join("\t");
        writeln!(file, "{}", trace_str).expect("Failed to write to file");
    }
}


fn crn_transition_probability(model: &AbstractVas, transition: &VasTransition, current_state: &DVector<i64>) -> f64 {
    ...
    for t in available_transitions {
        if let Some(vas_transition) = model.get_transition_from_id(t) {
            ...
            for (index, &current_value) in current_state.iter().enumerate() {
